...
try:
    ...
    print('Run the code with argument "list" for a list and usage of valid commands')
    exit()
#print(command)
...
if command == 'add':
    ...
    if acceptance == 'y' or acceptance == 'Y':
        ...
        with open('project_list.csv', mode='a+') as project_list:
            project_writer = csv.writer(project_list,
                                        delimiter=',',
                                        quotechar='"',
    ...
    try:
        ...
        print('Must add a project before start picking')
        exit()
    if len(projects) != 0:
        project_name = random.choice(projects)

        print('Chosen project is: ', project_name[0])
    ...
    print('Command List:')
    print(
        'add - Use to add a new project to the list\n \tSyntax - python3 project_picker.py add "project_name"'
    )
    print(
        'list - Use to see a list of commands\n \tSyntax - python3 project_picker.py list'
    )
    print(
        'pick - Use to select a project to work on\n \tSyntax - python3 project_picker.py pick'
    )

else:
    print('Invalid choice')
    print('Run the code with argunent "list" for a list and usage of valid commands')
